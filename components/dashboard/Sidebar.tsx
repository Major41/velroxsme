...
export function Sidebar() {
  ...
  const mainItems: SidebarItem[] = [
    ...
    { label: 'Sales', href: '/dashboard/sales', icon: <TrendingUp className="w-4 h-4" /> },
    { label: 'Expenses', href: '/dashboard/expenses', icon: <CreditCard className="w-4 h-4" /> },
    { label: 'Purchases', href: '/dashboard/purchases', icon: <ShoppingCart className="w-4 h-4" /> },
    { label: 'Customers', href: '/dashboard/customers', icon: <Users className="w-4 h-4" /> },
  ];

  const growthItems: SidebarItem[] = [
    { label: 'Marketing', href: '/dashboard/marketing', icon: <MessageCircle className="w-4 h-4" /> },
    { label: 'Reminders', href: '/dashboard/reminders', icon: <Bell className="w-4 h-4" />, badge: 4 },
  ];

  const settingsItems: SidebarItem[] = [
    // { label: 'Users', href: '/dashboard/users', icon: <Shield className="w-4 h-4" /> },
      ...
      >
        ...
        <div className="p-6 border-b border-slate-800">
          ...
          {business?.subscription_status && (
            <div className={`mt-2 px-2 py-1 rounded-md text-xs font-medium text-center ${
              business.subscription_status === 'active' 
                ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/30'
                : 'bg-red-500/20 text-red-300 border border-red-500/30'
            }`}>
              {business.subscription_status === 'active' ? 'Active Subscription' : 'Subscription Inactive'}
            </div>
          )}
        </div>
        ...
        <nav className="flex-1 overflow-y-auto p-4 space-y-8">
          {/* Main */}
          <div>
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3 px-2">Core</p>
          ...
          {/* Growth */}
          <div>
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3 px-2">Growth</p>
            <div className="space-y-1">
          ...
          {/* Settings */}
          <div>
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3 px-2">Settings</p>
            <div className="space-y-1">
